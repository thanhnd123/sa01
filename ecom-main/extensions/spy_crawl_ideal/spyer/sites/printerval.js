let Printerval = class extends Initial{
    constructor() {
        super();
    ...
    init() {
        if (document.querySelector('.exp-template') !== null) {
            ...
            button.addEventListener("click", (e) => {
                ...
                if (document.querySelector('#product-detail-content-wrapper')) {
                    this.getProduct()
                }
    ...
    getProduct() {
        let title = document.querySelector("h1.product-heading").innerText.trim();
        let images = [];
        // document.querySelectorAll(".product-slider-wrapper source").forEach(function (v, k) {
        //     let imgUrl = v.getAttribute('src');
        //     imgUrl = imgUrl.replace("/230x230x2", "/1024x1024x2");
        //     images.push(imgUrl);
        // });
        let banner = document.querySelector(".product-slider-wrapper source").getAttribute('srcset');
        if (!isURL(banner)) {
