let Crofil = class extends Initial{
    constructor() {
        super();
    ...
    init() {
        if (document.querySelector('.exp-template') !== null) {
            ...
            button.addEventListener("click", (e) => {
                ...
                if (document.querySelector('div.sticky-image-container')) {
                    this.getProduct()
                } else if (document.querySelector('.category-title') || location.pathname.index) {
    ...
    getProduct() {
        ...
        let images = [];
        document.querySelectorAll("div.product-images .left img").forEach(function (v, k) {
            let imgUrl = v.getAttribute('src');
            imgUrl = imgUrl.replace("/75xauto", "");
            images.push(imgUrl);
        });
