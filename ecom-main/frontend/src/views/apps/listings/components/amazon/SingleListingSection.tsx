...
const SingleListingSection: React.FC<SingleListingSectionProps> = ({
  onColumnsChange,
  ...
  const handleSingleProductUpload = (workbook: XLSX.WorkBook) => {
    try {
      ...
      if (jsonData.length > 3) {
        ...
        // Tạo columns cho single product
        const singleCols = headers.map((colName, idx) => {
        ...
        setColumns(singleCols)
        onColumnsChange(singleCols)

        // Set default values for all columns
        const initialColumnValues = singleCols.reduce(
          (acc, col) => {
  ...
  return (
    <Box sx={{ mt: 2 }}>
      <Typography variant='subtitle1' sx={{ mb: 2 }}>
        Single Product Template Columns
      </Typography>
      <Box sx={{ mb: 3 }}>
        <input
          type='file'
        ...
        <Button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Uploading...' : 'Upload Single Product Template File'}
        </Button>
