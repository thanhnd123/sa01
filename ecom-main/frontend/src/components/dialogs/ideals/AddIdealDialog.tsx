...
const AddIdealDialog = ({ open, onClose, onSuccess }: AddIdealDialogProps) => {
  ...
  const handleSubmit = async () => {
    ...
    const response = await axiosInstance.post(
      `${process.env.NEXT_PUBLIC_API_URL}/api/authenticated/ideals/create`,
      formDataToSend,
      {
        headers: {
          'Content-Type': 'multipart/form-data',
      ...
      if (response.data.response === 'success') {
        // Increment tag usage after successful creation
        if (formData.tags.length > 0) {
          incrementTagUsage(formData.tags)
        }
        toast.success('Ideal created successfully!')
        onSuccess()
        handleClose()
      } else {
        toast.error(response.data.message || 'Failed to create ideal')
      }
      setIsLoading(false)
    }).catch(error => {
      console.error('Error creating ideal:', error)
      toast.error(error.response?.data?.message || 'Failed to create ideal')
      setIsLoading(false)
    })
  }
  ...
  return (
    <Dialog
      onClose={handleClose}
      aria-labelledby='add-ideal-dialog-title'
      open={open}
      ...
      fullWidth
    >
      <DialogTitle id='add-ideal-dialog-title'>
        ...
        <Box sx={{ mt: 2 }}>
                ...
                {imagePreviews.length > 0 && (
                  <Box sx={{ mt: 2 }}>
                    <Typography variant='caption' sx={{ mb: 1, display: 'block' }}>
                      Preview ({imagePreviews.length}/{MAX_IMAGES}):
                    </Typography>
                    <Box sx={{
                      display: 'flex',
                      flexWrap: 'wrap',
          ...
          <Alert severity='info' sx={{ mt: 2 }}>
            <Typography variant='body2'>
              Fill in the information to create a new ideal. Banner is required. PNG, additional images and tags are optional.
            </Typography>
          </Alert>
        </Box>
      </DialogContent>
      <DialogActions sx={{ p: 3 }}>
        ...
        <Button
          ...
          disabled={isLoading}
        >
          {isLoading ? 'Creating...' : 'Create Ideal'}
        </Button>
      </DialogActions>
    </Dialog>
  )
