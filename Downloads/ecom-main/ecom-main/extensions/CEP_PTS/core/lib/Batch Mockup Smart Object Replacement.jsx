...
function parseFilePath(data, fileIndex, outputPathPrefix) {
  ...
  inputFilename = decodeURI(inputFilename);
  // replace []
  if (data.output.replace && data.output.replace.length > 0) {
    for (var i = 0; i < data.output.replace.length; i++) {
      var reg = new RegExp(data.output.replace[i], 'g');
      inputFilename = inputFilename.replace(reg, '');
    }
  }
  ...
  // 80 character limit.
  var limit = 80;
  var filename = inputFilename.substring(0, limit) + "/" + (data.doc.name + inputFilename).replace(/[^a-zA-Z0-9]/g, '').substring(0, limit).toLowerCase();
  // Just a thought....
...
function replaceLoopOptionsFiller(rawData) {
  ...
  var data = {
    output: {
      ...
      replace: rawData.output.replace.length > 0 ? rawData.output.replace : [],
    }
  };
